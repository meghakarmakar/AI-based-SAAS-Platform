    ...
    <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
      <div className='my-7 w-full'>
        ...
        <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
            ...
            {isAdmin && (
                <>
                    <NavLink to='/ai/admin/prompt-marketplace' onClick={()=> setSidebar(false)} className={({isActive})=> `px-3.5 py-2.5 flex items-center gap-3 rounded ${isActive ? 'bg-gradient-to-r from-[#3C81F6] to-[#9234EA] text-white' : ''}`}>
                        {({ isActive })=>(
                            <>
                            <ShoppingBag className={`w-4 h-4 ${isActive ? 'text-white' : ''}` } />
                            Prompt Management
                            </>
                        )}
                    </NavLink>
                    <NavLink to='/ai/admin/user-management' onClick={()=> setSidebar(false)} className={({isActive})=> `px-3.5 py-2.5 flex items-center gap-3 rounded ${isActive ? 'bg-gradient-to-r from-[#3C81F6] to-[#9234EA] text-white' : ''}`}>
                        {({ isActive })=>(
                            <>
                            <Shield className={`w-4 h-4 ${isActive ? 'text-white' : ''}` } />
                            User Management
                            </>
                        )}
                    </NavLink>
