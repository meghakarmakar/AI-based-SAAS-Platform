...
const Hero = () => {
    ...
    const handleGetStarted = () => {
        ...
        if (user) {
            ...
            openSignIn() // Open Clerk sign-in modal instead of navigating
        }
    }
    ...
    <div className='px-4 sm:px-20 xl:px-32 relative inline-flex flex-col w-full justify-center bg-[url(/gradientBackground.png)] bg-cover bg-no-repeat min-h-screen'>
        ...
        <div className='flex flex-wrap justify-center gap-4 text-sm max-sm:text-xs'>
            <button 
                ...
                disabled={!isLoaded}
                className='bg-primary text-white px-10 py-3 rounded-lg hover:scale-102 active:scale-95 transition cursor-pointer disabled:opacity-50'
            >
                Start creating now
            </button>
